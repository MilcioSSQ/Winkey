...
import { decryptJson, sha1Hex } from '../crypto/windkeyCrypto';
    ...
    const fetchPasswords = async () => {
      try {
        ...
        const decrypted = await Promise.all(response.data.map(async (p) => {
          ...
          return { ...entry, id: p.id, category_id: p.category_id };
        }));
  ...
  const checkForBreaches = async () => {
    ...
    try {
      for (const password of passwords) {
        const fullHash = await sha1Hex(password.password);
        const prefix = fullHash.slice(0, 5);
        const suffix = fullHash.slice(5);

        const response = await axios.post('/api/check-password-breach', { sha1Prefix: prefix });
        const match = response.data.suffixes.find((s) => s.suffix === suffix);

...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Best-effort exposure reduction: revealed plaintext passwords are
    // re-masked as soon as this tab loses focus or is hidden (alt-tab,
    // switching virtual desktops, minimizing). This narrows the window in
    // which a revealed password could be screenshotted or shoulder-surfed -
    // it cannot stop a screenshot/photo taken while the tab is still
    // visible and focused, which no web page can prevent.
    const hideRevealedPasswords = () => {
      setShowListPasswords({});
      setShowDialogPassword(false);
    };
    const handleVisibilityChange = () => {
      if (document.hidden) hideRevealedPasswords();
    };
    document.addEventListener('visibilitychange', handleVisibilityChange);
    window.addEventListener('blur', hideRevealedPasswords);
    return () => {
      document.removeEventListener('visibilitychange', handleVisibilityChange);
      window.removeEventListener('blur', hideRevealedPasswords);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('passwordsChanged', fetchPasswords);
    ...
    return () => {
      ...
      window.removeEventListener('passwordsChanged', fetchPasswords);
    };
  }, []);
  ...
  const fetchPasswords = async () => {
    try {
      ...
      const decrypted = await Promise.all(response.data.map(async (p) => {
        const entry = await decryptJson(userKey, p.encrypted_data, p.data_iv);
        return { ...entry, id: p.id, category_id: p.category_id, created_at: p.created_at, updated_at: p.updated_at };
      }));
  ...
  const handleOpen = async (password = null) => {
    if (password) {
      // The list is already decrypted client-side (fetchPasswords), no need
      // for a network round-trip to re-fetch and re-decrypt the same entry.
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data, iv } = await encryptJson(userKey, formData);
      if (editingPassword) {
        await axios.put(`/api/passwords/${editingPassword.id}`, { encrypted_data: data, data_iv: iv });
                ...
                <CardContent>
                  ...
                  {password.username && (
                    <Typography
                      ...
                      sx={{ mb: 2 }}
                    >
                      Benutzername: {password.username}
                    </Typography>
                  )}

                  {showListPasswords[password.id] && (
                    <Typography
                      variant="body2"
                      sx={{
                        mb: 2,
